...
def test_compare_baseline_workflow_reports_zero_mismatches(tmp_path: Path):
    workflow_path = Path("examples/compare-state-summary-baseline.yaml")
    ...
    rows = _read_rows(Path(result["output_dir"]) / "comparison.csv")
    assert rows
